...
@File    ：utils_for_RIB_NASC.py
@Author  ：Mingtong Zhang
@Date    ：2025/12/1 20:05 
'''
import torch
...
class SynSamplingLoss(nn.Module):
    def __init__(self, p_sent, p_token, num_sampling):
        super().__init__()
        self.p_sent = p_sent
        self.p_token = p_token
        self.num_sampling = num_sampling

    def forward_jensen(self, logits, targets_real, targets_syn, tgt_pad_mask, pad_idx):
        batch_size, num_candidates, seq_len = targets_syn.shape
        device = targets_real.device

        # 预先计算log_softmax以提高效率和稳定性
        pad_mask = tgt_pad_mask.reshape(-1).bool()
        logits = logits.reshape(-1, logits.size(-1))[pad_mask]
        log_probs = F.log_softmax(logits, dim=-1)
        log_sum = torch.zeros_like(log_probs)[:, 0]
        ...
        for _ in range(self.num_sampling):
            ...
            # 收集对应目标的log概率
            log_probs_t = log_probs.gather(dim=-1, index=valid_targets.unsqueeze(-1)).squeeze(-1)
            log_sum = log_sum + log_probs_t

        log_sum = log_sum / self.num_sampling

        return (-log_sum).mean()

    def forward(self, logits, targets_real, targets_syn, pad_idx, tau):
        batch_size, num_candidates, seq_len = targets_syn.shape
        device = targets_real.device
        ...
        for _ in range(self.num_sampling):
            ...
            # 收集对应目标的log概率
            log_probs_t = log_probs.gather(dim=-1, index=targets.unsqueeze(-1)).squeeze(-1)

            # 处理填充位置，log(1)=0
            log_probs_t.masked_fill_(targets == pad_idx, 0.0)

            # 计算各样本的log概率和
            log_probs_sum = log_probs_t.view(batch_size, -1).sum(dim=1)
            log_sum.append(log_probs_sum)

        # 堆叠并计算log-sum-exp
        stacked = torch.stack(log_sum, dim=0)  # [num_sampling, bs]
        stacked = stacked / tau
        max_log = stacked.max(dim=0).values
        # 向量化计算，提高效率
        exp_term = torch.exp(stacked - max_log).sum(dim=0)
        log_sum = tau * (max_log + torch.log(exp_term))
        return (-log_sum).mean()
...
def train_step(model, sent, sent_syn, pad, end, opt, channel, device, args):
    # 训练步骤开始
    model.train()
    model.club_net.eval()
    model.mine_net.eval()
    ...
    elif args.sampling_method == 'jensen':
        loss_fn_syn = SynSamplingLoss(p_sent=args.p_sent, p_token=args.p_token, num_sampling=args.num_sampling)
        mi_s_z_lb_n = loss_fn_syn.forward_jensen(pred, tgt_real, syn_real, tgt_pad_mask, pad)
    elif args.sampling_method == 'log_sum_exp':
        loss_fn_syn = SynSamplingLoss(p_sent=args.p_sent, p_token=args.p_token, num_sampling=args.num_sampling)
        tau = args.tau * torch.ones_like(tgt_real)[:, 0]
        mi_s_z_lb_n = loss_fn_syn.forward(pred, tgt_real, syn_real, pad, tau)
    elif args.sampling_method == 'log_sum_exp_dynamic':
        loss_fn_syn = SynSamplingLoss(p_sent=args.p_sent, p_token=args.p_token, num_sampling=args.num_sampling)
        tau = args.tau * torch.ones_like(tgt_real)[:, 0]
        tau = tau * torch.log(1 + len_sent.squeeze(-1))
        mi_s_z_lb_n = loss_fn_syn.forward(pred, tgt_real, syn_real, pad, tau)
    else:
        mi_s_z_lb_n = None
...
def eval_step(model, sent, sent_syn, pad, end, channel, device, args):
    # 训练步骤开始
    model.eval()
