...
@File    ：utils_for_RIB_DeepSC.py
@Author  ：Mingtong Zhang
@Date    ：2025/12/11 14:45 
'''
import torch
...
def rand_syn_generate(sent, sent_syn, p_sent, p_token, pad_idx):
    batch_size, num_candidates, seq_len = sent_syn.shape
    device = sent.device

    # 生成句子级别的替换掩码
    sent_replace_mask = torch.rand(batch_size, device=device) < p_sent  # (batch_size,)
    sent_replace_mask_expanded = sent_replace_mask.unsqueeze(1).expand(batch_size, seq_len)  # (batch, seq)

    # 生成词元级别的替换掩码
    token_replace_prob = torch.rand((batch_size, seq_len), device=device) < p_token

    # 1. 随机选择候选索引
    replace_idx = torch.randint(0, num_candidates, (batch_size, seq_len), device=device)

    # 2. 收集选中的候选
    selected_candidates = torch.gather(sent_syn, 1, replace_idx.unsqueeze(1).expand(-1, -1, seq_len)).squeeze(1)

    # 3. 处理选中pad的情况
    first_candidate = sent_syn[:, 0, :]  # 获取第一个候选
    ...
    targets = torch.where(total_replace_mask, corrected_candidates, sent)

    return targets


class RandSynReplace_v1(nn.Module):
    def __init__(self, p_sent, p_token):
        super().__init__()
        self.p_sent = p_sent
        self.p_token = p_token

    def forward(self, logits, targets_syn, tgt_pad_mask):
        # 提取有效目标（非填充位置）
        valid_targets = targets_syn[tgt_pad_mask.bool()]
...
def train_step(model, sent, sent_syn, pad, end, opt, channel, device, args):
    ...
    sent_no_end = torch.where(condition_tensor, pad, sent)
    src = sent_no_end[:, 1: -1]
    syn_sent = rand_syn_generate(sent, sent_syn, args.p_sent, args.p_token, pad)
    syn_sent_no_end = torch.where(condition_tensor, pad, syn_sent)

    tgt_inp = syn_sent_no_end[:, :-1]
    tgt_real = sent[:, 1:]
    syn_real = syn_sent[:, 1:]
    ...
    loss_fn_syn = RandSynReplace_v1(p_sent=args.p_sent, p_token=args.p_token)
    mi_s_z_lb_n = loss_fn_syn(pred, syn_real, tgt_pad_mask)
