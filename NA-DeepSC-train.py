...
@File    ：NA-DeepSC-train.py
@Author  ：Mingtong Zhang
@Date    ：2025/11/27 21:48 
'''
import os
import pickle
import time
...
from models.NA_DeepSC import NA_DeepSC
from utils.utils_for_NA_DeepSC import train_step, eval_step
...
parser = get_train_parser(case='NA-DeepSC', channel=channel, version=None, config_number=config_number)
...
def train_eval(model, num_epoch, lr, optim, init, scheduler_type, eval_interval):
    ...
    for epoch_index in range(start_epoch, num_epoch + 1):
        ...
        for sents in train_bar:
            ...
            batch_loss_train = train_step(model=model, sent=sents, pad=pad_idx, end=end_idx, opt=optimizer, channel=args.channel, device=device, unk=unk_idx, args=args)
        ...
        if epoch_index % eval_interval == 0:
            ...
            with torch.no_grad():
                for sents_eval in eval_bar:
                    ...
                    batch_loss_eval = eval_step(model=model, sent=sents_eval, pad=pad_idx, end=end_idx, unk=unk_idx, channel=args.channel, device=device, args=args)
...
if __name__ == '__main__':
    ...
    # 创建网络模型
    model = NA_DeepSC(num_layers=args.num_layers, src_vocab_size=num_vocab, tgt_vocab_size=num_vocab, model_dim=args.d_model, channel_dim=args.d_channel,
