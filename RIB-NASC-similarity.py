...
@File    ：RIB-NASC-similarity.py
@Author  ：Mingtong Zhang
@Date    ：2025/12/4 22:19 
'''
import pickle
import time
import os
...
from models.RIB_NASC import RIB_NASC_v0
from utils.utils_general import setup_seed, Timer, Data_savemat, SimScore, SeqtoText
from utils.utils_for_RIB_NASC import na_decode
...
parser = get_test_parser(case='RIB-NASC', channel=channel, version=version, train_config_number=train_config_number, test_config_number=test_config_number,
        ...
        for loop in range(args.test_loop):
            ...
            for snr in SNR:
                ...
                for sents_test in test_bar:
                    ...
                    out = na_decode(model=model, sent=sents_test, pad=pad_idx, end=end_idx, snr=snr, channel=args.channel, device=device, args=args)
...
if __name__ == '__main__':
    ...
    model = RIB_NASC_v0(num_layers=args.num_layers, src_vocab_size=num_vocab, tgt_vocab_size=num_vocab, model_dim=args.d_model, channel_dim=args.d_channel,
