...
@File    ：RIB-NA-DeepSC-similarity.py
@Author  ：Mingtong Zhang
@Date    ：2025/12/11 11:06 
'''
import pickle
import time
import os
...
from models.RIB_NA_DeepSC import RIB_NA_DeepSC_v0
from utils.utils_general import setup_seed, Timer, Data_savemat, SimScore, SeqtoText
from utils.utils_for_RIB_NA_DeepSC import na_decode
...
channel = 'AWGN'
best_mode = True
train_config_number = '0.1'
test_config_number = '0.1'
beta = 0.0001

device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

parser = get_test_parser(case='RIB-NA-DeepSC', channel=channel, version=version, train_config_number=train_config_number, test_config_number=test_config_number,
                         trained_epoch=None, best_mode=best_mode, beta=beta)
        ...
        for loop in range(args.test_loop):
            ...
            for snr in SNR:
                ...
                for sents_test in test_bar:
                    ...
                    out = na_decode(model=model, sent=sents_test, pad=pad_idx, end=end_idx, snr=snr, channel=args.channel, device=device,
                                    unk=unk_idx, args=args)
...
if __name__ == '__main__':
    ...
    model = RIB_NA_DeepSC_v0(num_layers=args.num_layers, src_vocab_size=num_vocab, tgt_vocab_size=num_vocab, model_dim=args.d_model, channel_dim=args.d_channel,
